...
# handle tag
if GIT_TERMINAL_PROMPT=0 eval "$GIT_TOOL_FOR_EVAL" $GIT_CREDENTIALS ls-remote --tags "$1" | grep -q "refs/tags/$3$"; then
  if [ ! -z $4 ]; then
...
# handle branch
elif GIT_TERMINAL_PROMPT=0 eval "$GIT_TOOL_FOR_EVAL" $GIT_CREDENTIALS ls-remote --heads "$1" | grep -q "refs/heads/$3"; then
  if [ ! -z $4 ]; then
